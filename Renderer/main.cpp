...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_LBUTTONUP:
		dev_app.setMUp((uint8_t)lParam);
		break;
	case WM_LBUTTONDOWN:
		dev_app.setMDown((uint8_t)lParam);
		break;
	
	case WM_MOUSEMOVE:
		dev_app.setMPosition(GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam));
		break;
	default:
...
MSG begin_main_loop()
{
	...
	while (true)
	{
		...
		{
			renderer.default_view = dev_app.update(renderer.default_view);
			renderer.draw();
		}
	}
